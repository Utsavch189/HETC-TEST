...
def register(request,qr):
    ...
    if request.method == 'POST':
        with_qr = ((str(request.path).split("/"))[2]).split("=")[1]
        ...
        if record_is_duplicate(fullname, gurdian, email):
            ...
            board_name=board, appeared_wbjee_jeeMain=entrance, preferred_stream=stream,with_qr=with_qr)
            student.save()

            activate_url = get_current_site(request).domain+"/"+"login"


            subject = "Thank You for registration"
            body = f'''
            Hello!
            You are successfully registered for HETC Scholarship Test 2023.\n
